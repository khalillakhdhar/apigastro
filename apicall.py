...
def lookup():
    ...
    if not image_url or not question:
        return jsonify({"error": "image_url et question sont requis"}), 400

    # Extraire img_id depuis l'URL
    try:
        filename = os.path.basename(urlparse(image_url).path)
        img_id = os.path.splitext(filename)[0]
    except:
        return jsonify({"error": "Impossible d'extraire img_id depuis image_url"}), 400

    # Chercher la ligne correspondante
    subset = df[df["img_id"] == img_id]
    if subset.empty:
        return jsonify({"error": f"Aucune entrée trouvée pour l'image {img_id}"}), 404
    ...
    # Correspondance floue de la question
    match = get_close_matches(question, subset["question"].tolist(), n=1, cutoff=0.7)
    if match:
        matched_question = match[0]
        answer = subset[subset["question"] == matched_question]["answer"].iloc[0]
